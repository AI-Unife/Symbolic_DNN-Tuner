...
t(0.60)::action(dec_fc_layers, latency).
t(0.60)::action(dec_fc_layers, model_size).
t(0.60)::action(dec_conv_block, latency).
t(0.60)::action(dec_conv_block, model_size).
t(0.60)::action(dec_conv_layers, latency).
t(0.60)::action(dec_conv_layers, model_size).
t(0.50)::action(remove_residual,latency).
t(0.50)::action(remove_residual,model_size).

%problem rules
0.70::action(new_fc_layers, underfitting):- problem(underfitting), \+problem(latency), \+problem(model_size).
0.70::action(inc_conv_layers, underfitting):- problem(underfitting), \+problem(latency), \+problem(model_size).
...
0.60::action(dec_fc_layers, latency):- problem(latency).
0.60::action(dec_fc_layers, model_size):- problem(model_size).
0.60::action(dec_conv_block, latency):- problem(latency).
0.60::action(dec_conv_block, model_size):- problem(model_size).
0.60::action(dec_conv_layers, latency):- problem(latency).
0.60::action(dec_conv_layers, model_size):- problem(model_size).
