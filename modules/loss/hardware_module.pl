...
%expanding old rules
0.70::action(new_fc_layers, underfitting):- problem(underfitting), \+problem(out_range).
0.70::action(new_conv_layers, underfitting):- problem(underfitting), \+problem(out_range).
