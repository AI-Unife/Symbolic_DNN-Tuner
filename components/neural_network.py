...
class neural_network:
    ...
    def insert_conv_section(self, model, params, n_conv):
        ...
        # if self.dnet.max_conv:
        #     self.tot_conv = self.dnet.max_conv
        ...
        for i in range(2):
            ...
            conv_layers = [k for k in params if re.match(r'new_conv_\d+$', k)]

            # Find the key with the highest number
            last_layer = max(conv_layers, key=lambda k: int(re.search(r'\d+$', k).group()))
            new_section += [Conv2D(params[last_layer], (3,3)), Activation(params['activation'])]
    ...
    def insert_fc_section(self, model, params, n_fc):
        ...
        # n_layer = self.dnet.count_layer_type(model, 'Dense')
        # if n_layer >= self.tot_fc:
        #     print(colors.FAIL, "Max number of dense layers reached", colors.ENDC)
        #     print(colors.FAIL, "Dense layers: ", n_layer, " Max dense layers: ", self.tot_fc, colors.ENDC)
        #     return model 
        ...
        new_section = [Dense(params[last_layer]),
