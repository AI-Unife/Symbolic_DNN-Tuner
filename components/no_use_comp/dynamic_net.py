...
class dynamic_net:
    ...
    def build_model(self, model, model_dict):
        ...
        try:
            ...
            print(f"\n#### Error during model creation, too much conv layer!! ####\n")
            # print(f"\n#### {model_dict} \n Conv layers: {self.max_conv} ####\n")
            return model

    def count_layer_from_dict(self, model_dict, layer_type):
        """
        method used to count how many layers of a certain type are in the model
        :param model_dict type: model_dict in which to count number of 'type' layers
        :return: number of layers of the specified type
        """
        return [(layer_type in model_dict[i][1]) for i in model_dict.keys()].count(True)
