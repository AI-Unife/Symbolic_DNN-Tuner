...
def train_best_model_if_required(exp_dir: Path):
    ...
    dataset_name = exp_dir.name.split("_")[5]  # estrai dataset dal nome cartella
    ...
    callbacks = [
        tf.keras.callbacks.EarlyStopping(monitor="val_acc", mode="max", patience=30, restore_best_weights=True)
    ]
...
def parse_experiment_meta(log_text: str, all_modules: Iterable[str]) -> Tuple[str, Dict[str, float | str], Dict[str, bool]]:
    ...
    dataset_name = dataset.group(1).strip().lower().replace("-", " ") if dataset else ""
...
def run(
    ...
    for child in sorted(base_dir.iterdir()):
        ...
        if train:
            acc_test, loss_test = train_best_model_if_required(
                exp_dir=child)
            info.best_metrics["Test Accuracy after fine-tuning"] = acc_test if acc_test is not None else np.nan
        if info:
